...
def train_one_epoch(epoch_index, tb_writer, train_load, model, optimizer, loss_fn):
    running_loss = 0.
    last_loss = 0.
...
def train(epoch_num, train_load, val_load, model, optimizer, loss_fn, name):
    ...
    for epoch in range(epoch_num):
        model.train(True)
        avg_loss = train_one_epoch(epoch_number, writer, train_load, model, optimizer, loss_fn)
